...
class BranchEventHead(nn.Module):
    """
    Branch-wise binary crossing head.
    Input : future_feats [B, K, D]
    Output: branch_logits [B, K, 1]
    """
...
class MultiFutureCrossingDecisionModel(nn.Module):
    """
    Video -> Backbone -> Temporal Encoder -> MultiFuture Predictor
          -> BranchEventHead (crossing)
          -> ClipBinaryHead  (early-decision)
    ...
    def forward(self, video):
        ...
        early_logit = self.early_head(clip_feat)                  # [B,1]
